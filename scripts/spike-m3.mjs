#!/usr/bin/env node
/**
 * SPIKE-1's Windows-side wrapper — THROWAWAY. Delete this and `src/engine/spike/`
 * when M3's preparation pipeline is built.
 *
 *   node scripts/spike-m3.mjs --address 192.0.2.10 --fixture "C:\CastGood\.fixture"
 *
 * From WSL, do not call this directly: `scripts/spike-m3.sh` runs it on the Windows
...
function cannotRun(message) {
  process.stderr.write(`\n[spike-m3] cannot run: ${message}\n`);
  process.exit(COULD_NOT_RUN);
}

if (process.platform !== 'win32') {
  cannotRun(
    `SPIKE-1 drives a real Chromecast, and the TV has to open a TCP connection back to
  this machine — neither survives WSL2's NAT (CLAUDE.md). This is ${process.platform}.
  From WSL:  scripts/spike-m3.sh --address 192.0.2.10 --fixture "C:\\CastGood\\.fixture"`,
  );
}

const entry = path.join(root, 'src/engine/spike/m3-cli.ts');
if (!existsSync(entry)) {
  cannotRun(`no spike entry point at ${path.relative(root, entry)} — has the spike been deleted?`);
}

const pkg = JSON.parse(readFileSync(path.join(root, 'package.json'), 'utf8'));
const outfile = path.join(root, 'dist/spike/spike-m3.cjs');
...
process.stderr.write(`[spike-m3] args: ${JSON.stringify(args)}\n`);
