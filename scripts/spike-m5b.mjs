#!/usr/bin/env node
/**
 * SPIKE-4's Windows-side wrapper — THROWAWAY. Delete this, `scripts/spike-m5b.sh` and
 * `src/engine/spike/m5b*.ts` once M5a's volume control is built.
 *
 *   node scripts/spike-m5b.mjs --device "<device name>" --file "C:\path\to\film.mp4"
 *
 * From WSL, do not call this directly: `scripts/spike-m5b.sh` runs it on the Windows
 ...
 *
 * EXIT CODES — propagated from `m5b-cli.ts`, which has three rather than two.
...
function cannotRun(message) {
  process.stderr.write(`\n[spike-m5b] cannot run: ${message}\n`);
  process.exit(COULD_NOT_RUN);
}

if (process.platform !== 'win32') {
  cannotRun(
    `SPIKE-4 drives a real Chromecast, and the TV has to open a TCP connection back to
  this machine — neither survives WSL2's NAT (CLAUDE.md). This is ${process.platform}.
  From WSL:  scripts/spike-m5b.sh --device "<device name>" --file "C:\\path\\to\\film.mp4"`,
  );
}

const entry = path.join(root, 'src/engine/spike/m5b-cli.ts');
if (!existsSync(entry)) {
  cannotRun(`no spike entry point at ${path.relative(root, entry)} — has the spike been deleted?`);
}

const pkg = JSON.parse(readFileSync(path.join(root, 'package.json'), 'utf8'));
const outfile = path.join(root, 'dist/spike/spike-m5b.cjs');
...
process.stderr.write(`[spike-m5b] args: ${JSON.stringify(args)}\n`);
