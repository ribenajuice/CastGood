#!/usr/bin/env node
/**
 * SPIKE-2's Windows-side wrapper — THROWAWAY. Delete this and `src/engine/spike/`
 * when M2's session supervisor is built.
 *
 *   node scripts/spike-m2.mjs --address 192.0.2.10 --file "C:\path\to.mp4" --probe socket
 *
 * From WSL, do not call this directly: `scripts/spike-m2.sh` runs it on the Windows
 * side, which is the only place a Chromecast is reachable.
 *
 * It is a copy of `scripts/selftest.mjs`'s shape on purpose — refuse anywhere but
...
function cannotRun(message) {
  process.stderr.write(`\n[spike-m2] cannot run: ${message}\n`);
  process.exit(COULD_NOT_RUN);
}

if (process.platform !== 'win32') {
  cannotRun(
    `SPIKE-2 drives a real Chromecast, and the TV has to open a TCP connection back to
  this machine — neither survives WSL2's NAT (CLAUDE.md). This is ${process.platform}.
  From WSL:  scripts/spike-m2.sh --address 192.0.2.10 --file "C:\\Users\\You\\film.mp4"`,
  );
}

const entry = path.join(root, 'src/engine/spike/cli.ts');
if (!existsSync(entry)) {
  cannotRun(`no spike entry point at ${path.relative(root, entry)} — has the spike been deleted?`);
}

const pkg = JSON.parse(readFileSync(path.join(root, 'package.json'), 'utf8'));
const outfile = path.join(root, 'dist/spike/spike-m2.cjs');
...
process.stderr.write(`[spike-m2] args: ${JSON.stringify(args)}\n`);
