...
You will have an installer called `CastGood-Setup-1.1.0.exe`. Double-click it.
...
`CastGood-Setup-1.1.0.exe /NOFIREWALL` skips the firewall step entirely.
...
Either way the result is `dist/installer/CastGood-Setup-1.1.0.exe`. Copy it to the Windows side and double-click it.
